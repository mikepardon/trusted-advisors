<template>
  <div class="duel-kingdoms">
    ...
    >
      ...
      <canvas :ref="el => setCanvasReference(kingdom.player_number, el)" class="kingdom-dice-canvas" />
      ...
      <h5 class="kingdom-username kingdom-name-clickable" @click="$emit('showCharacter', kingdom.player_number)">{{ kingdom.character_name }}</h5>
...
interface DiceInstance {
  isReady: () => boolean;
  init: (canvas: HTMLCanvasElement, options: { diceSize: number }) => Promise<boolean>;
...
const emit = defineEmits<{
  ...
  "diceAnimationComplete": [payload: { playerNumber: number; timestamp: number }];
}>();
...
const kingdoms = computed<DisplayKingdom[]>(() =>
  playerKingdoms.map((kingdom) => {
    const playerNumber = kingdom.player?.player_number ?? kingdom.player_number ?? 0;
    const isRealBot = Boolean(kingdom.player?.is_bot) && !kingdom.player?.user;
    const characterName = kingdom.player?.character?.name ?? "Advisor";
    const displayUsername = isRealBot ? "Bot" : (kingdom.player?.user?.name || "Player");
    return {
      ...kingdom,
      player_number: playerNumber,
      character_name: characterName,
      username: playerNumber === myPlayerNumber && isSinglePlayerDuel ? `${displayUsername} (YOU)` : displayUsername,
    };
...
watch(() => playerKingdoms, (newKingdoms) => {
  if (!newKingdoms) {
    return;
  }
...
watch(() => diceAnimationTrigger, async (trigger) => {
  if (!trigger) {
    return;
  }
...
function getKingdomAnim(playerNumber: number): string {
  return playerKingdomStyleData[playerNumber]?.css_vars?.border_anim || "none";
}
