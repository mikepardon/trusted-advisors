<template>
    <div class="adv-modal-overlay" @click.self="$emit('close')">
        <div class="adv-modal">
            <button class="adv-modal-close" @click="$emit('close')">
                &times;
            </button>
            ...
            <div class="adv-portrait-wrap">
                <img
                    :src="
                        advisor.character.image_url || '/images/character.png'
                    "
            ...
            <p v-if="advisor.incarnation > 1" class="adv-incarnation">
                Incarnation {{ advisor.incarnation }}
            </p>
            ...
            <div class="adv-xp-section">
                ...
                <span
                    v-if="advisor.level < (advisor.max_level || 8)"
                    class="adv-xp-label"
                >
                    Level {{ advisor.level }} &mdash;
                    {{ advisor.xp - advisor.xp_for_current_level }} /
                    {{
                        advisor.xp_for_next_level - advisor.xp_for_current_level
                    }}
                    XP
                </span>
                <span v-else class="adv-xp-label adv-xp-max"
                    >Level {{ advisor.max_level || 8 }} &mdash; MAX</span
                >
            </div>
            ...
            <div v-if="hasBonuses" class="adv-bonuses-section">
                ...
                <div class="adv-bonus-list">
                    <span
                        v-if="advisor.extra_item_slots > 0"
                        class="adv-bonus-tag"
                        >+{{ advisor.extra_item_slots }} Item Slot{{
                            advisor.extra_item_slots > 1 ? "s" : ""
                        }}</span
                    >
                    <span v-if="advisor.card_redraws > 0" class="adv-bonus-tag"
                        >{{ advisor.card_redraws }} Card Redraw{{
                            advisor.card_redraws > 1 ? "s" : ""
                        }}</span
                    >
                    <span
                        v-for="(val, stat) in advisor.passive_bonuses"
                        :key="stat"
                        class="adv-bonus-tag"
                        >+{{ val }} {{ capitalize(stat) }}</span
                    >
                </div>
            </div>
            ...
            <div class="adv-tree-section">
                ...
                <div class="adv-tree">
                    <div
                        v-for="lvl in (advisor.max_level || 8) - 1"
                        :key="lvl"
                        class="adv-tree-node"
                        :class="treeNodeClass(lvl + 1)"
                    >
                        <span class="adv-tree-level">Lv{{ lvl + 1 }}</span>
                        <template v-if="upgradeAtLevel(lvl + 1)">
                            <div class="adv-tree-upgrade-info">
                                <span class="adv-tree-chosen">{{
                                    upgradeAtLevel(lvl + 1).option_name
                                }}</span>
                                <span class="adv-tree-desc">{{
                                    upgradeAtLevel(lvl + 1).option_description
                                }}</span>
                                <span
                                    v-if="
                                        upgradeChoiceLabel(
                                            upgradeAtLevel(lvl + 1),
                                        )
                                    "
                                    class="adv-tree-choice"
                                    >{{
                                        upgradeChoiceLabel(
                                            upgradeAtLevel(lvl + 1),
                                        )
                        ...
                        <span
                            v-else-if="
                                advisor.level >= lvl + 1 &&
                                advisor.pending_upgrades > 0 &&
                                isNextPending(lvl + 1)
                            "
                            class="adv-tree-available"
                            >Available</span
                        >
        ...
        <Transition name="imm-fade">
            <div
                v-if="showImmortaliseConfirm"
                class="adv-immortalise-overlay"
                @click.self="showImmortaliseConfirm = false"
            >
                <div class="adv-immortalise-modal">
                    <div class="adv-immortalise-icon">&#10047;</div>
                    <p class="adv-immortalise-title">
                        Immortalise this advisor?
                    </p>
                    <p class="adv-immortalise-desc">
                        They will reset to Level 1 with a new title. Previous
                        upgrades will be lost for this incarnation.
                    </p>
                    <p
                        class="adv-immortalise-cost"
                        :class="{ short: !canAffordImmortalise }"
                    >
                        Cost: &#129689; {{ immortaliseCost
                        }}<span v-if="!canAffordImmortalise">
                            — not enough coins</span
                        >
                    </p>
                    <div class="adv-immortalise-btns">
                        <button
                            class="btn-adv-immortalise-yes"
                            :disabled="immortalising || !canAffordImmortalise"
                            @click="doImmortalise"
                        >
                            {{ immortalising ? "Immortalising..." : "Confirm" }}
                        </button>
                        <button
                            class="btn-adv-immortalise-no"
                            :disabled="immortalising"
                            @click="showImmortaliseConfirm = false"
                        >
                            Cancel
                        </button>
                    </div>
...
const canAffordImmortalise = computed(
    () => (auth.state.user?.coins ?? 0) >= immortaliseCost.value,
);
...
const xpPercent = computed<number>(() => {
    ...
    return Math.min(
        100,
        ((advisor.xp - advisor.xp_for_current_level) / range) * 100,
    );
});

const hasBonuses = computed<boolean>(
    () =>
        advisor.extra_item_slots > 0 ||
...
function upgradeAtLevel(level: number): AdvisorUpgrade | undefined {
    return (advisor.upgrades || []).find(
        (upgrade) => upgrade.chosen_at_level === level,
    );
}
...
function upgradeChoiceLabel(upgrade: AdvisorUpgrade): string {
    ...
    if (type === "bump_two_dice_faces" && choice.faces) {
        return choice.faces
            .map(
                (face) =>
                    `Die ${(face.die_index ?? 0) + 1} Face ${(face.face_index ?? 0) + 1}`,
            )
            .join(", ");
    }
    return "";
...
async function doImmortalise(): Promise<void> {
    ...
    try {
        const response = await axios.post<{
            message: string;
            new_coins?: number;
        }>(`/api/my-advisors/${advisor.id}/immortalise`);
