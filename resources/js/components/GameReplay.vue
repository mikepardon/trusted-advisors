<template>
    <div class="replay-page">
        ...
        <div v-else-if="error" class="card-panel replay-unavailable">
            <div class="replay-unavailable-icon">&#9876;</div>
            <h3 class="replay-unavailable-title">
                {{ incomplete ? "Campaign Unfinished" : "Replay Unavailable" }}
            </h3>
            <p class="replay-unavailable-text">
                {{
                    incomplete
                        ? "This campaign was abandoned before it finished, so there is no replay to watch."
                        : error
                }}
            </p>
            <button
                ...
                @click="$router.push(shareToken ? '/' : '/campaigns')"
            >
                &larr; {{ shareToken ? "Home" : "Back to Campaigns" }}
            </button>
        </div>
        ...
        <template v-else>
            ...
            <div class="card-panel replay-info">
                <div class="replay-meta">
                    <span
                        class="type-badge"
                        :class="'type-' + (game.game_type || 'cooperative')"
                    >
                        {{ game.game_type === "duel" ? "Duel" : "Classic" }}
                    </span>
                    <span
                        class="mode-badge"
                        :class="'mode-' + (game.game_mode || 'single')"
                    >
                        {{
                            {
                                single: "Solo",
                    ...
                    <span class="replay-rounds"
                        >{{ totalRoundsPlayed }} rounds played</span
                    >
                </div>
            ...
            <div class="round-nav">
                ...
                <span class="round-label"
                    >Round {{ currentRound }} / {{ totalRoundsPlayed }}</span
                >
                <button
                    class="nav-btn"
                    :disabled="currentRound >= totalRoundsPlayed"
                    @click="currentRound++"
                >
                    &rarr; Next
                </button>
            ...
            <!-- Event Banner -->
            <div
                v-if="currentEventData && currentEventData.name"
                class="card-panel event-banner"
            >
                ...
                <p
                    v-if="currentEventData.description"
                    class="event-description"
                >
                    {{ currentEventData.description }}
                </p>
                <div
                    v-if="currentEventData.stat_modifiers"
                    class="event-modifiers"
                >
                    <span
            ...
            <!-- Cards Dealt Section -->
            <div
                v-if="currentRoundHands && currentRoundHands.length > 0"
                class="card-panel cards-section"
            >
                ...
                <div class="cards-grid">
                    ...
                    >
                        <div class="dealt-card-header">
                            <span class="dealt-card-name">{{
                                hand.card?.title || "Card"
                            }}</span>
                            <span
                                v-if="hand.role"
                                class="role-badge"
                                :class="'badge-' + hand.role"
                                >{{ hand.role }}</span
                            >
                        </div>
                        <div
                            v-if="hand.card?.difficulty"
                            class="dealt-card-difficulty"
                        >
                            Difficulty: {{ hand.card.difficulty }}
                        </div>
                        <div
                            v-if="hand.card?.positive_effects"
                            class="dealt-card-effects"
                        >
                            <span
                                v-for="(val, stat) in filterStatEffects(
                                    hand.card.positive_effects,
                                )"
                                :key="'pos-' + stat"
                                class="mini-effect effect-positive"
                            >
                                {{ formatStatName(stat) }} +{{ val }}
                            </span>
                        </div>
                        <div
                            v-if="hand.card?.negative_effects"
                            class="dealt-card-effects"
                        >
                            <span
                                v-for="(val, stat) in filterStatEffects(
                                    hand.card.negative_effects,
                                )"
                                :key="'neg-' + stat"
                                class="mini-effect effect-negative"
                            >
                                {{ formatStatName(stat) }} {{ val }}
                            </span>
                        </div>
                        <div
                            v-if="hand.player?.character?.name"
                            class="dealt-card-player"
                        >
            ...
            <div
                v-if="roundItemsReceived.length > 0"
                class="card-panel items-section"
            >
                ...
                <div class="special-tags-grid">
                    ...
                    >
                        ...
                        <span class="tag-player">{{
                            getPlayerNameById(pi.game_player_id)
                        }}</span>
                    </button>
                </div>
            </div>
            ...
            <div
                v-if="roundCurses.length > 0"
                class="card-panel curses-section"
            >
                ...
                <div class="special-tags-grid">
                    ...
                    >
                        ...
                        <span class="tag-player">{{
                            getPlayerNameById(pc.game_player_id)
                        }}</span>
                    </button>
                </div>
            </div>
            ...
            <div class="round-details">
                <div
                    v-if="!currentRoundData || currentRoundData.length === 0"
                    class="no-data"
                >
                    No data for this round.
                </div>
                <div
                    v-for="result in currentRoundData"
                    :key="result.id"
                    class="result-card card-panel"
                >
                    <div class="result-header">
                        <strong class="card-name">{{
                            resultCardName(result)
                        }}</strong>
                        <span v-if="result.player" class="player-tag">{{
                            result.player?.character?.name || "Player"
                        }}</span>
                    </div>
                    ...
                    <div class="result-body">
                        ...
                        <template v-if="isDuelResult(result)">
                            <div
                                v-for="(pr, pi) in result.dice_results"
                                :key="pi"
                                class="dice-section"
                            >
                                <span class="sub-label"
                                    >{{ pr.character_name }}:</span
                                >
                                <span
                            ...
                            <div class="dice-section">
                                <span class="sub-label"
                                    >vs Difficulty
                                    {{
                                        result.stat_totals?.total_difficulty
                                    }}</span
                                >
                                <span
                                    class="dice-outcome"
                        ...
                        <template v-else-if="result.dice_results">
                            <div class="dice-section">
                                ...
                                <span
                                    v-for="(d, i) in result.dice_results"
                                    :key="i"
                                    class="die-face"
                                    >{{ d }}</span
                                >
                        ...
                        <div v-if="hasEffects(result)" class="effects-section">
                            ...
                            <div class="effects-list">
                                <template
                                    v-if="Array.isArray(result.effects_applied)"
                                >
                                    <span
                                        v-for="(
                                            eff, i
                                        ) in result.effects_applied"
                                        :key="i"
                                        class="effect-tag"
                                        >{{ eff }}</span
                                    >
                        ...
                        >
                            ...
                            <div class="effects-list">
                                <template
                                    v-for="(
                                        effect, i
                                    ) in result.special_effects"
                                    :key="i"
                                >
                                    <button
                                        v-if="
                                            effect.type === 'draw_item' ||
                                            effect.type === 'item_blocked' ||
                                            effect.type === 'discard_item'
                                        "
                                    ...
                                    <span
                                        v-else
                                        class="effect-tag effect-special"
                                    >
                                        {{
                        ...
                        <div
                            v-if="result.kingdom_snapshot"
                            class="kingdom-snapshot"
                        >
                            ...
                            <div class="stat-chips">
                                ...
                                >
                                    {{ formatStatName(stat) }}: {{ val }}
                                    <span
                                        v-if="statDelta(stat, val) !== 0"
                                        class="stat-delta"
                                    >
                                        ({{ statDelta(stat, val) > 0 ? "+" : ""
                                        }}{{ statDelta(stat, val) }})
                                    </span>
                                </span>
                            </div>
        ...
        <teleport to="body">
            <transition name="detail-fade">
                <div
                    v-if="detailOverlay.show"
                    class="detail-overlay"
                    @click.self="detailOverlay.show = false"
                >
                    <div class="detail-viewer">
                        <button
                            class="detail-close"
                            @click="detailOverlay.show = false"
                        >
                            &times;
                        </button>
                        ...
                        >
                            <div class="detail-parchment">
                                ...
                                <h3 class="parchment-title">
                                    {{ detailOverlay.data.title || "Card" }}
                                </h3>

                                <div
                                    v-if="detailOverlay.data.category"
                                    class="parchment-category"
                                >
                                    {{ detailOverlay.data.category }}
                                </div>
                                ...
                                <p
                                    v-if="detailOverlay.data.description"
                                    class="parchment-desc"
                                >
                                    {{ detailOverlay.data.description }}
                                </p>

                                <div class="parchment-stat-row">
                                    <span class="parchment-stat difficulty-stat"
                                        >Difficulty:
                                        {{
                                            detailOverlay.data.difficulty
                                        }}</span
                                    >
                                </div>

                                <div class="parchment-divider divider-thin">
                                    <span class="divider-diamond small"
                                        >&#8226;</span
                                    >
                                </div>
                                ...
                                <!-- Positive effects -->
                                <div
                                    v-if="detailOverlay.data.positive_effects"
                                    class="parchment-effects"
                                >
                                    ...
                                    <div class="effects-chips">
                                        ...
                                        <span
                                            v-if="
                                                detailOverlay.data
                                                    .positive_effects.draw_item
                                            "
                                            class="stat-chip chip-special"
                                            >Draw Item</span
                                        >
                                        <span
                                            v-if="
                                                detailOverlay.data
                                                    .positive_effects.draw_curse
                                            "
                                            class="stat-chip chip-curse"
                                            >Draw Curse</span
                                        >
                                        <span
                                            v-if="
                                ...
                                <!-- Negative effects -->
                                <div
                                    v-if="detailOverlay.data.negative_effects"
                                    class="parchment-effects"
                                >
                                ...
                                <div
                                    v-if="
                                        detailOverlay.data.positive_flavor ||
                                        detailOverlay.data.negative_flavor
                                    "
                                    class="parchment-flavor"
                                >
                                    <p
                                        v-if="
                                            detailOverlay.data.positive_flavor
                                        "
                                        class="flavor-text flavor-positive"
                                    >
                                        {{ detailOverlay.data.positive_flavor }}
                                    </p>
                                    <p
                                        v-if="
                                            detailOverlay.data.negative_flavor
                                        "
                                        class="flavor-text flavor-negative"
                                    >
                                        {{ detailOverlay.data.negative_flavor }}
                                    </p>
                                </div>
                        ...
                        >
                            <div class="detail-parchment">
                                <div class="parchment-ornament">&#9876;</div>
                                <h3 class="parchment-title">
                                    {{ detailOverlay.data.name || "Item" }}
                                </h3>

                                <div class="tag-row">
                                    <span
                                        v-if="detailOverlay.data.is_consumable"
                                        class="type-tag consumable-tag"
                                        >Consumable</span
                                    >
                                    <span v-else class="type-tag reusable-tag"
                                        >Reusable</span
                                    >
                                    <span
                                        v-if="
                                            detailOverlay.data.target ===
                                            'opponent'
                                        "
                                        class="type-tag opponent-tag"
                                        >Targets Opponent</span
                                    >
                                </div>
                                ...
                                <div
                                    v-if="itemEffectSummary(detailOverlay.data)"
                                    class="effects-chips centered"
                                >
                                    <span
                                        class="stat-chip"
                                        :class="
                                            itemChipClass(detailOverlay.data)
                                        "
                                    >
                                        {{
                                            itemEffectSummary(
                                ...
                                <div
                                    v-if="detailOverlay.data.effect_type"
                                    class="parchment-meta"
                                >
                        ...
                        <template
                            v-if="
                                detailOverlay.type === 'curse' &&
                                detailOverlay.data
                            "
                        >
                            <div class="detail-parchment parchment-curse">
                                <div class="parchment-ornament curse-ornament">
                                    &#9760;
                                </div>
                                <h3 class="parchment-title curse-title">
                                    {{ detailOverlay.data.name || "Curse" }}
                                </h3>

                                <div class="tag-row">
                                    <span class="type-tag cursed-tag"
                                        >Curse</span
                                    >
                                </div>

                                <div class="parchment-divider curse-divider">
                                    <span class="divider-diamond">&#9830;</span>
                                </div>
                                ...
                                <div
                                    class="parchment-divider divider-thin curse-divider"
                                >
                                    <span class="divider-diamond small"
                                        >&#8226;</span
                                    >
                                ...
                                <div
                                    v-if="detailOverlay.data.negative_effect"
                                    class="parchment-effects"
                                >
                                    <span class="effects-label curse-label"
                                        >Penalty:</span
                                    >
                                    <div class="effects-chips centered">
                                        <span class="stat-chip chip-negative">{{
                                            describeCurseEffect(
                                                detailOverlay.data
                                                    .negative_effect,
                                            )
                                ...
                                <div
                                    v-if="detailOverlay.data.positive_effect"
                                    class="parchment-effects"
                                >
                                    <span class="effects-label">Reward:</span>
                                    <div class="effects-chips centered">
                                        <span class="stat-chip chip-positive">{{
                                            describeCurseEffect(
                                                detailOverlay.data
                                                    .positive_effect,
                                            )
...
const detailOverlay = reactive<DetailOverlay>({
    show: false,
    type: undefined,
    data: undefined,
});

// Distinguish "the game was abandoned mid-campaign" from a genuine load error,
// so we can show a friendly ending instead of a raw message.
const incomplete = computed(() =>
    (error.value ?? "").toLowerCase().includes("not completed"),
);
...
const roundItemsReceived = computed(() =>
    allPlayerItems.value.filter(
        (item) => item.acquired_round === currentRound.value,
    ),
);
const roundItemsUsed = computed(() =>
    allPlayerItems.value.filter(
        (item) => item.used_round === currentRound.value,
    ),
);
...
const roundCurses = computed(() =>
    allPlayerCurses.value.filter(
        (curse) => curse.acquired_round === currentRound.value,
    ),
);
...
function isDuelResult(result: RoundResult): boolean {
    return (
        Array.isArray(result.dice_results) &&
...
function getPlayerNameById(gamePlayerId: number): string {
    ...
    const player = game.value.players.find(
        (playerRecord) => playerRecord.id === gamePlayerId,
    );
...
function openItemByName(itemName?: string): void {
    ...
    const found = allPlayerItems.value.find(
        (playerItem) => playerItem.item?.name === itemName,
    );
    if (found?.item) {
...
function describeCurseEffect(effect?: CurseEffect): string {
    ...
    switch (type) {
        case "stat_penalty": {
            return `${value} ${effect.stat ?? "stat"} per round`;
        }
        ...
        case "recover_die": {
            return "Recover a die";
        }
        case "remove_curse": {
            return "Remove this curse";
        }
        default: {
...
.replay-unavailable-icon {
    font-size: 2.4rem;
    color: var(--accent-gold);
    filter: drop-shadow(0 2px 6px rgba(0, 0, 0, 0.5));
}

.replay-unavailable-title {
    font-family: "Cinzel", serif;
    color: var(--accent-gold);
    font-size: 1.2rem;
}

.replay-unavailable-text {
    color: var(--text-secondary);
    font-style: italic;
    line-height: 1.5;
    max-width: 340px;
...
.replay-unavailable .btn-primary {
    margin-top: 8px;
}
...
.dealt-card.role-positive {
    border-color: rgba(74, 138, 58, 0.3);
}
.dealt-card.role-negative {
    border-color: rgba(160, 48, 32, 0.3);
}
