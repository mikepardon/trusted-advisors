...
router.beforeEach(async (to, from, next) => {
    ...
    if (
        (requiresAuth && !auth.state.user) ||
        (requiresAdmin && !auth.state.user?.is_admin)
    ) {
        next("/");
        ...
        next(to.path === "/choose-username" ? undefined : "/choose-username");
    } else if (!requiresAdmin && auth.state.user?.needs_advisors) {
        next(to.path === "/choose-advisors" ? undefined : "/choose-advisors");
    } else {
        next();
    }
});
